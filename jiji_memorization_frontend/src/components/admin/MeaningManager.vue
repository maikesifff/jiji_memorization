  ...
  <div class="meaning-manager">
    ...
    <div class="table-container">
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="meaning in filteredMeanings" :key="meaning.id">
            <td>{{ meaning.id }}</td>
            <td>{{ meaning.wordText || '-' }}</td>
            <td>{{ meaning.pos || '-' }}</td>
            <td>{{ meaning.content || '-' }}</td>
            <td>{{ formatDate(meaning.createdAt) }}</td>
            <td>
              <button @click="editMeaning(meaning)" class="edit-btn">编辑</button>
              <button @click="deleteMeaning(meaning.id)" class="delete-btn">删除</button>
            </td>
          </tr>
        </tbody>
      </table>
      
             <div v-if="filteredMeanings.length === 0" class="no-data">
         暂无数据
       </div>
    ...
    <div v-if="showAddModal || showEditModal" class="modal-overlay">
      <div class="modal">
        ...
        <form @submit.prevent="handleSubmit" class="modal-form">
          ...
          <div class="form-group">
            <label>词性</label>
            <select v-model="formData.pos">
              <option value="">请选择词性</option>
              <option 
                v-for="(chineseName, posType) in posTypes" 
                :key="posType" 
                :value="posType"
              >
                {{ chineseName }} ({{ posType }})
...
export default {
  ...
  setup() {
    ...
    const meaningToDelete = ref(null)
    ...
    const filteredMeanings = computed(() => {
      if (!searchTerm.value) return meanings.value
      // 如果正在搜索，返回搜索结果；否则返回当前页数据
      if (isSearching.value) {
        return searchResults.value
      }
      // 如果不在搜索状态，但在当前页数据中找到了匹配项，则返回过滤结果
      const filtered = meanings.value.filter(meaning => {
        const wordText = meaning.wordText || ''
        const content = meaning.content || ''
    ...
    const performSearch = async () => {
      ...
      try {
        ...
        const response = await api.get(`/api/meanings/search?keyword=${encodeURIComponent(searchTerm.value)}&page=0&size=${pageSize.value}`)
    ...
    const searchPage = async (page) => {
      ...
      try {
        ...
        const response = await api.get(`/api/meanings/search?keyword=${encodeURIComponent(searchTerm.value)}&page=${page}&size=${pageSize.value}`)
    ...
    const fetchMeanings = async () => {
      try {
        loading.value = true
        const response = await api.get(`/api/meanings?page=${currentPage.value}&size=${pageSize.value}`)
        // 处理分页数据，提取content数组
        if (response.data && response.data.content) {
          meanings.value = response.data.content
          // 设置分页信息
          pagination.value = {
            currentPage: response.data.currentPage,
            totalPages: response.data.totalPages,
          ...
          meanings.value = response.data || []
          pagination.value = {
            currentPage: 0,
            totalPages: 1,
            totalItems: meanings.value.length,
            hasNext: false,
            hasPrevious: false
          }
        }
    ...
    const fetchPosTypes = async () => {
      try {
        const response = await api.get('/api/meanings/pos-types')
        if (response.data && response.data.status === 'success') {
          posTypes.value = response.data.data || {}
        } else {
          console.error('Failed to fetch pos types:', response.data?.message)
        }
      } catch (error) {
        console.error('Failed to fetch pos types:', error)
        // 如果API调用失败，使用默认的词性类型
        posTypes.value = {
          'n.': '名词',
          'v.': '动词',
    ...
    const addMeaning = async () => {
      try {
        const response = await api.post('/api/meanings', formData)
        ...
        if (response.data && response.data.status === 'success') {
          ...
          // 显示错误信息
          alert(response.data?.message || '添加释义失败')
        }
      } catch (error) {
        ...
        if (error.response && error.response.data && error.response.data.message) {
          ...
          alert('添加释义失败: ' + error.message)
        }
      }
    }
    
    // 编辑释义
    const editMeaning = async (meaning) => {
      formData.id = meaning.id
      formData.wordId = meaning.wordId
      formData.pos = meaning.pos || ''
      formData.content = meaning.content || ''
      ...
      if (meaning.wordText) {
        ...
        try {
          const response = await api.get(`/api/words/${meaning.wordId}`)
    ...
    const updateMeaning = async () => {
      try {
        const response = await api.put(`/api/meanings/${formData.id}`, formData)
        ...
        if (response.data && response.data.status === 'success') {
          ...
          // 显示错误信息
          alert(response.data?.message || '更新释义失败')
        }
      } catch (error) {
        ...
        if (error.response && error.response.data && error.response.data.message) {
          ...
          alert('更新释义失败: ' + error.message)
        }
      }
    }
    
    // 删除释义
    const deleteMeaning = (meaningId) => {
      meaningToDelete.value = meanings.value.find(m => m.id === meaningId)
    ...
    const confirmDelete = async () => {
      try {
        await api.delete(`/api/meanings/${meaningToDelete.value.id}`)
        const index = meanings.value.findIndex(m => m.id === meaningToDelete.value.id)
        if (index !== -1) {
