  ...
  <div class="phrase-manager">
    ...
    <div class="table-container">
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="phrase in filteredPhrases" :key="phrase.id">
            <td>{{ phrase.id }}</td>
            <td>{{ phrase.wordText || '-' }}</td>
            <td>{{ phrase.phraseText || '-' }}</td>
            <td>{{ phrase.translation || '-' }}</td>
            <td>{{ formatDate(phrase.createdAt) }}</td>
            <td>
              <button @click="editPhrase(phrase)" class="edit-btn">编辑</button>
              <button @click="deletePhrase(phrase.id)" class="delete-btn">删除</button>
            </td>
          </tr>
        </tbody>
      </table>
      
             <div v-if="filteredPhrases.length === 0" class="no-data">
         暂无数据
       </div>
...
export default {
  ...
  setup() {
    ...
    const filteredPhrases = computed(() => {
      if (!searchTerm.value) return phrases.value
      // 如果正在搜索，返回搜索结果；否则返回当前页数据
      if (isSearching.value) {
        return searchResults.value
      }
      // 如果不在搜索状态，但在当前页数据中找到了匹配项，则返回过滤结果
      const filtered = phrases.value.filter(phrase => {
        const wordText = phrase.wordText || ''
        const content = phrase.phraseText || ''
    ...
    const performSearch = async () => {
      ...
      try {
        ...
        const response = await api.get(`/api/phrases/search?keyword=${encodeURIComponent(searchTerm.value)}&page=0&size=${pageSize.value}`)
    ...
    const searchPage = async (page) => {
      ...
      try {
        ...
        const response = await api.get(`/api/phrases/search?keyword=${encodeURIComponent(searchTerm.value)}&page=${page}&size=${pageSize.value}`)
    ...
    const fetchPhrases = async () => {
      try {
        loading.value = true
        const response = await api.get(`/api/phrases?page=${currentPage.value}&size=${pageSize.value}`)
        // 处理分页数据，提取content数组
        if (response.data && response.data.content) {
          phrases.value = response.data.content
          // 设置分页信息
          pagination.value = {
            currentPage: response.data.currentPage,
            totalPages: response.data.totalPages,
          ...
          pagination.value = {
            currentPage: 0,
            totalPages: 1,
            totalItems: phrases.value.length,
            hasNext: false,
            hasPrevious: false
          }
        }
    ...
    const addPhrase = async () => {
      try {
        const response = await api.post('/api/phrases', formData)
        ...
        if (response.data && response.data.status === 'success') {
          ...
          // 显示错误信息
          alert(response.data?.message || '添加短语失败')
        }
      } catch (error) {
    ...
    const editPhrase = async (phrase) => {
      formData.id = phrase.id
      formData.wordId = phrase.wordId
      formData.phraseText = phrase.phraseText || ''
      formData.translation = phrase.translation || ''
      
      // 直接从当前数据中获取单词信息，避免额外的API调用
      if (phrase.wordText) {
                  editingWordInfo.value = {
            wordText: phrase.wordText
          }
      } else {
        // 如果当前数据中没有单词信息，尝试从API获取
        try {
          const response = await api.get(`/api/words/${phrase.wordId}`)
    ...
    const updatePhrase = async () => {
      try {
        const response = await api.put(`/api/phrases/${formData.id}`, formData)
        ...
        if (response.data && response.data.status === 'success') {
          ...
          // 显示错误信息
          alert(response.data?.message || '更新短语失败')
        }
      } catch (error) {
    ...
    const deletePhrase = (phraseId) => {
      phraseToDelete.value = phrases.value.find(p => p.id === phraseId)
    ...
    const confirmDelete = async () => {
      try {
        await api.delete(`/api/phrases/${phraseToDelete.value.id}`)
        const index = phrases.value.findIndex(p => p.id === phraseToDelete.value.id)
        if (index !== -1) {
