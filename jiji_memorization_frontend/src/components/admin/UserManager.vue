  ...
  setup() {
    ...
    const filteredUsers = computed(() => {
      ...
      const filtered = users.value.filter(user => 
        ...
        (user.nickname && user.nickname.toLowerCase().includes(searchTerm.value.toLowerCase()))
      )
    ...
    const performSearch = async () => {
      ...
      try {
        ...
        const response = await api.get(`/api/users/search?keyword=${encodeURIComponent(searchTerm.value)}&page=0&size=${pageSize.value}`)
    ...
    const searchPage = async (page) => {
      ...
      try {
        ...
        const response = await api.get(`/api/users/search?keyword=${encodeURIComponent(searchTerm.value)}&page=${page}&size=${pageSize.value}`)
    ...
    const fetchUsers = async () => {
      try {
        loading.value = true
        const response = await api.get(`/api/users?page=${currentPage.value}&size=${pageSize.value}`)
        // 处理分页数据，提取content数组
        if (response.data && response.data.content) {
    ...
    const updateUser = async () => {
      try {
        ...
        if (!userData.password) {
          delete userData.password // 如果密码为空，不更新密码
        }
        
        const response = await api.put(`/api/users/${formData.id}`, userData)
        ...
        if (response.data && response.data.status === 'success') {
          ...
          // 显示错误信息
          alert(response.data?.message || '更新用户失败')
        }
      } catch (error) {
